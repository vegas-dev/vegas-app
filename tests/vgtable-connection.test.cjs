...
test('connects VGTable and leaves VGDynamicTable outside the main bundle', () => {
	...
	assert.match(fixedColumnStyles, /tbody tr > \[data-vg-table-fixed-side\],[\s\S]*?box-shadow: inset 0 0 0 100vmax transparent;[\s\S]*?transition: var\(--vg-table-transition\);/);
	assert.doesNotMatch(fixedColumnStyles, /transition: box-shadow var\(--vg-table-transition\);/);
