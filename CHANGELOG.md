# VEGAS-APP 1.4.8 (Август, 20, 2026)
## Изменения
* В `VGFiles` исправлена валидация одиночной замены файла: ошибка превышения допустимого размера больше не очищается повторной обработкой и остаётся видимой пользователю.
* Добавлен регрессионный тест `VGFiles` для отклонённого файла и последующей успешной замены допустимым файлом.
* Команда `npm test` теперь запускает все тестовые файлы `tests/*.test.cjs`, а не только тесты `VGTooltip`.
* Версия пакета подготовлена к выпуску `1.4.8`.

---

# VEGAS-APP 1.4.7 (Август, 20, 2026)
## Изменения
* В `VGDynamicTable` количество skeleton-строк теперь сохраняет текущую высоту таблицы: сначала учитываются отрисованные строки и уже показанный skeleton, а значение по умолчанию уменьшено с 10 до 5.
* В `VGDynamicTable` убраны радиусы крайних ячеек и clone sticky header, чтобы фон отсортированной колонки не получал отдельные закругления; внешний радиус обычной таблицы сохранён.
* В `VGFiles` клик по списку уже выбранных файлов внутри label-контейнера больше не открывает системный диалог выбора файла повторно.
* Версия пакета и собранные файлы подготовлены к выпуску `1.4.7`.

---

# VEGAS-APP 1.4.6 (Август, 20, 2026)
## Изменения
* `VGTooltip` теперь отслеживает удаление открытого trigger-элемента из DOM и полностью освобождает tooltip, observer, window listeners, таймеры и связанные атрибуты.
* Повторное позиционирование больше не выполняется для отключённого trigger-элемента; orphan-tooltip очищаются при открытии следующей подсказки.
* Быстрый уход курсора с trigger до истечения задержки показа теперь отменяет pending tooltip и не оставляет зависшую подсказку.
* Добавлены регрессионные тесты жизненного цикла `VGTooltip` для динамической/AJAX-перерисовки и обычных сценариев `show()` / `hide()`.
* Версия пакета подготовлена к выпуску `1.4.6`.
