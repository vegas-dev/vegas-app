# VEGAS-APP 1.5.9 (Сентябрь, 1, 2026)
## Изменения
* **VGTable Fixed Columns**: синхронизирована анимация hover фиксированных ячеек и строки; сохранены непрозрачный фон, полосатые строки и состояния `selected` / `selected:hover` без просвечивания прокручиваемого содержимого.
* **VGAlert Overlay**: окно подтверждения внутри длинной прокручиваемой `VGModal` остаётся в видимой области и располагается с отступом от её верхнего края; поведение overlay в `VGSidebar` не изменено.
* **VGNav**: исправлено плавное переключение соседних dropdown первого уровня — новое меню полностью открывается до начала закрытия предыдущего, а устаревшие отложенные переходы не закрывают повторно выбранный пункт.
* Добавлены регрессионные тесты `VGTable`, `VGAlert` и `VGNav`, версия пакета подготовлена к выпуску `1.5.9`.

---

# VEGAS-APP 1.5.8 (Сентябрь, 1, 2026)
## Новые фичи
* **VGSelect**: добавлен параметр `height` и Data API `data-height` для ограничения высоты списка; значение `true` использует стандартные `300px`, число задаёт собственный предел, `false` снимает ограничение.

## Изменения
* **VGTable Fixed Header**: геометрия колонок теперь заново измеряется после ручного обновления или AJAX-замены строк, учитывает нативное `table-layout: auto`, добавление колонок и уменьшение ширины содержимого, сохраняя минимумы служебных колонок с пустыми заголовками.
* Исправлены отступы элементов управления сортировкой `VGTable`: размерные CSS-переменные объявляются на самой таблице и корректно учитывают одинарную и множественную сортировку.
* Сброс фильтров `VGTable` синхронизирует визуальное состояние `VGSelect` с нативным `<select>` без повторного запуска фильтрации.
* Добавлены регрессионные тесты `VGSelect` и `VGTable`, обновлены production-сборки `build/vgapp.*`, версия пакета — `1.5.8`.

---

# VEGAS-APP 1.5.7 (Август, 28, 2026)
## Изменения
* **VGFiles**: исправлен выбор миниатюр внешних файлов — `src` используется как изображение только для графических файлов, отдельная обложка `image` имеет приоритет, обложки аудио сохраняются.
* Документы и архивы в списках и dropzone отображают SVG-иконки; ошибка загрузки миниатюры возвращает иконку файла.
* Добавлены регрессионные тесты внешних документов, изображений, обложек и fallback; обновлена production-сборка, версия пакета — `1.5.7`.

---

# VEGAS-APP 1.5.6 (Август, 27, 2026)
## Новые фичи
* **Responsive**: добавлен публичный сервис брейкпоинтов с экспортом `Responsive` и `DEFAULT_BREAKPOINTS`, диапазонами `xs`–`xxl`, глобальными и локальными границами, наследованием профилей, подписками на смену диапазона и определением viewport и touch-ввода.
* **VGTable**: добавлены адаптивные профили пагинации для количества кнопок, расположения и выравнивания панелей, выбора размера страницы и быстрого перехода; доступны методы `getResponsiveState()` / `refreshResponsive()` и событие `responsivechange.vg.table`.
* **VGTable**: новая настройка `pagination.maxButtons` и атрибут `data-pagination-max-buttons` ограничивают суммарное количество номеров страниц и многоточий без учёта кнопок «назад» / «вперёд».

## Изменения
* Адаптивное обновление пагинации `VGTable` сохраняет страницу, размер выборки, строки, URL и сохранённые настройки без повторных remote-запросов; поддержаны восстановление фокуса и ввода в сохранившихся полях, переключение внешних панелей и очистка подписок.
* В `VGNav` исправлены позиционирование вложенных и вертикальных меню, закрытие всех вложенных уровней, сохранение обработчиков для открытого родителя и защита повторно открытого меню от старого callback закрытия; гамбургер больше не обрабатывается как dropdown-ссылка.
* В `VGRangeSlider` исправлены синхронизация внешних hidden-input, пересчёт отметок шкалы при изменении ширины, актуальность статуса одиночного слайдера и округление по шагу без выхода за `max`.
* В `VGRollup` исправлены приоритет `data-lang`, вызов `callbacks.collapse`, применение `-webkit-line-clamp` и сохранение пользовательских inline-стилей при раскрытии.
* Добавлены регрессионные тесты Responsive, VGNav, VGRangeSlider и VGRollup; расширены тесты VGTable для адаптивной локальной и remote-пагинации.
* Обновлены собранные `build/vgapp.js` и source map, версия пакета в `package.json` — `1.5.6`.

---

# VEGAS-APP 1.5.1 (Август, 23, 2026)
## Изменения
* Геометрия Fixed Header разделяет явно заданные жёсткие и гибкие колонки: служебные размеры сохраняются, а свободное пространство пропорционально распределяется между остальными без ложного горизонтального overflow.
* Добавлен регрессионный тест смешанной геометрии колонок, обновлены production-сборки, версия пакета подготовлена к выпуску `1.5.1`.

---

# VEGAS-APP 1.5.0 (Август, 23, 2026)
## Новые фичи
* **VGFiles**: добавлена настройка `customButtons.hideOnUploadState`, которая по умолчанию скрывает пользовательские действия файла в состояниях `pending`, `loading` и `failing`; поведение можно отключить через публичную конфигурацию.

## Изменения
* В Fixed Header `VGTable` ширины колонок теперь измеряются по реальным строкам тела и жёстко синхронизируются с `th` и обоими `colgroup`; пустые заголовки сохраняют ширину заполненных колонок.
* Fixed Header автоматически пересчитывает геометрию через `ResizeObserver` при изменении ширины layout, включая схлопывание боковых колонок; повторные уведомления объединяются через `requestAnimationFrame`.
* Синхронизация Fixed Header больше не принимает строки `empty`, skeleton и другие служебные состояния за источник геометрии, поэтому таблица сохраняет стабильные ширины при удалённой перерисовке.
* В page-режиме тело таблицы не создаёт собственную вертикальную прокрутку; слои header и body изолированы, чтобы фиксированные ячейки строк не перекрывали заголовок.
* Добавлены регрессионные тесты VGFiles и VGTable, обновлены production-сборки `build/vgapp.*`, версия пакета подготовлена к выпуску `1.5.0`.

---

# VEGAS-APP 1.4.9 (Август, 22, 2026)
## Новые фичи
* **VGTable**: добавлен новый публичный модуль таблиц на базе обычной HTML-разметки с Data API и JavaScript API, локальной и серверной сортировкой, мультисортировкой, пагинацией, поиском, фильтрами и синхронизацией состояния с URL.
* **VGTable**: реализованы фиксированный заголовок и колонки, управление шириной, порядком и видимостью колонок, перестановка и выбор строк, многоуровневое раскрытие строк, состояния `empty` / `filtered-empty` / `error`, skeleton-загрузка, локали `ru` / `en` и сохранение настроек в `localStorage`.
* **VGTable Remote**: добавлена работа с JSON-данными и готовой серверной разметкой, настраиваемыми путями ответа и параметрами запроса, GET/POST, экспортом, повтором запроса и кешированием ответа.
* **VGAlert**: добавлен компактный размер `size: "compact"` для modal, overlay и dropdown-режимов.
* **VGNestable**: добавлены публичные методы `expandItem()` / `collapseItem()` / `expandAll()` / `collapseAll()`, Data API для массового раскрытия, события `collapse` / `expand`, доступное состояние пустого списка и сериализация дополнительных `data-*` атрибутов.

## Изменения
* `VGTable` подключён к стандартному реестру и корневому экспорту VGApp; добавлена публичная SCSS-точка `vgapp/table`, подробная документация и регрессионные тесты основных локальных и Remote-сценариев.
* Устаревший `VGDynamicTable` удалён из исходников, стандартного bundle, корневых JS/SCSS-входов и публичных экспортов пакета.
* В `VGDropdown` добавлен безопасный повторный `initAll()`, устранено дублирование глобальных обработчиков, улучшены очистка экземпляра и перепозиционирование внутри clipping-контейнеров и `offsetParent`.
* В `VGFiles` исправлены состояния статистики и информации после отклонения всех файлов, внутренний sortable drag больше не воспринимается как загрузка; унифицированы компактные кнопки удаления и оформление действий dropzone.
* В `VGFilePreview` восстановлены элементы управления видео-плейлистом, список соседних видео формируется непосредственно перед открытием, а доступность image preview корректно учитывает публичную настройку группы.
* В `VGLoadMore` исправлены начальный offset, контейнер infinite scroll, объединение пользовательских AJAX-параметров, событие успешной статической загрузки, скрытие триггера после окончания данных и его восстановление после AJAX-ошибки.
* В `VGFormSender` восстановлены обязательные классы alert для collapse-ответов; в базовом модуле GET-параметры перенесены в URL и добавлена обработка ошибок с корректным завершением loader.
* Обновлены стили и drag-and-drop поведение `VGNestable`, включая полное превью ветки, корректный placeholder и возврат элемента на исходную позицию по `Escape`.
* В корневом `README.md` список модулей синхронизирован с `VGTable` и добавлен раздел для проектов сообщества; собранные `build/vgapp.*` обновлены, версия пакета подготовлена к выпуску `1.4.9`.
